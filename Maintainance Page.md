You are getting the CSRF mismatch because while you added the token to your HTML `<meta>` tag, your JavaScript `fetch()` call isn't actually sending it to the server! Laravel requires the token to be sent back in the request headers.

I have added the `X-CSRF-TOKEN` header to your JavaScript `fetch()` call. I also fixed a small typo in your logo div where `#background` was written instead of `background`.
...
define('CFG', [
    ...
    'admin_email'    => 'support@yourdomain.com',  // BCC / admin copy
...
if ($_SERVER['REQUEST_METHOD'] === 'POST'
    ...
    $submission = [
        ...
        'timestamp' => date('c'),
        'ip'        => $ip,
        'userAgent' => substr(clean($body['userAgent'] ?? '', 150), 0, 150),
    ];
...
### ⚠️ Important Note on CSRF & Session Expiration

Adding the `X-CSRF-TOKEN` header fixes the immediate issue. However, **if a user leaves the maintenance page open in their browser for a long time, their Laravel session will expire, and the token will become invalid.** When they finally fill out the form and submit it, they will get a `419 Page Expired` / CSRF mismatch error again.

Because this is a **maintenance page** (where users are likely to leave the tab open for hours/days), the best practice is to **disable CSRF checking entirely for this route**.

To do that, open `app/Http/Middleware/VerifyCsrfToken.php` in your Laravel project and add your route to the exceptions:

```php
protected $except = [
    '/maintenance',       // Add the exact URL of your maintenance page here
    '/maintenance.php',   // And the .php version if you use it
];
```